...
if have_any:
    ...
    # Show minimum ending values across all windows (worst case result for each source)
    min_values = {}
    if src_kind in ("LBM", "BOTH") and df_lbm is not None and allocation_cols_lbm:
        min_val_global = float("inf")
        for col in allocation_cols_lbm:
            evs = simulate_ending_values_lumpsum(df_lbm[col], int(num_years), int(row_increment))
            if evs:
                min_val_global = min(min_val_global, min(evs))
        if min_val_global != float("inf"):
            min_values["Global"] = min_val_global
    if src_kind in ("SPX", "BOTH") and df_spx is not None and allocation_cols_spx:
        min_val_spx = float("inf")
        for col in allocation_cols_spx:
            evs = simulate_ending_values_lumpsum(df_spx[col], int(num_years), int(row_increment))
            if evs:
                min_val_spx = min(min_val_spx, min(evs))
        if min_val_spx != float("inf"):
            min_values["SP500"] = min_val_spx

    if min_values:
        st.markdown("#### Minimum Ending Values (worst-case window)")
        for k, v in min_values.items():
            st.write(f"{k}: {v:,.2f} × initial investment")
