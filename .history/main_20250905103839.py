...
if have_any:
    ...
    # Header hover help via column_config (works with st.data_editor)
    col_config = {
        "Allocation": st.column_config.TextColumn(
            "Allocation",
            help="Stock/bond mix (e.g., 60% Equity).",
        ),
    }
    if "Global" in display_results.columns:
        col_config["Global"] = st.column_config.NumberColumn(
            "Global",
            help="Required lump sum using Global (historical audit).",
            format="%,.0f",  # already formatted as text with $; kept number fallback
        )
    if "Global CAGR" in display_results.columns:
        col_config["Global CAGR"] = st.column_config.TextColumn(
            "Global CAGR",
            help="Implied worst‑window CAGR for Global over the selected years.",
        )
    if "SP500" in display_results.columns:
        col_config["SP500"] = st.column_config.NumberColumn(
            "SP500",
            help="Required lump sum using S&P 500 (historical audit).",
            format="%,.0f",
        )
    if "SP500 CAGR" in display_results.columns:
        col_config["SP500 CAGR"] = st.column_config.TextColumn(
            "SP500 CAGR",
            help="Implied worst‑window CAGR for S&P 500 over the selected years.",
        )

    # Show as read‑only grid with header tooltips
    st.data_editor(
        display_results,
        hide_index=True,
        disabled=True,
