...
if have_any:
    ...
    if src_kind == "LBM":
        ...
        # Use only present columns to avoid category mismatches
        present_lbm = [c for c in allocation_cols_lbm if c in results["Allocation"].unique()]
        present_spx = [c for c in allocation_cols_spx if c in results["Allocation"].unique()]
        order = present_lbm + present_spx

    # Sort by raw codes, then build friendly Portfolio Name labels
    ...
    # Build Global/SP500 display names
    pretty_lbm_global = {}
    for k, v in pretty_lbm.items():
        if "Fixed" in v:
            pretty_lbm_global[k] = v.replace("Fixed", "Global Fixed")
        else:
            pretty_lbm_global[k] = v.replace(" Equity", " Global Equity")

    pretty_spx_sp500 = {k: v.replace(" Equity", " SP500 Equity") for k, v in pretty_spx.items()}

    # Combine maps; unknowns fall back to raw header
    name_map = {**pretty_lbm_global, **pretty_spx_sp500}
    results["Portfolio Name"] = results["Allocation"].map(name_map).fillna(results["Allocation"])
    ...
    desired_cols = ["Portfolio Name", "Required Lump Sum"]
