...
if st.button("Compute required lump sum"):
    if df is None or not allocation_cols:
        ...
        if pdf_path:
            ...
            # Save to ./static and provide a regular link (more reliable than pop-ups)
            os.makedirs("static", exist_ok=True)
            safe_name = re.sub(r'[^A-Za-z0-9._-]', '_', file_label)
            static_path = os.path.join("static", safe_name)
            try:
                with open(static_path, "wb") as f:
                    f.write(pdf_bytes)
                st.markdown(
                    f'<a href="{static_path}" target="_blank" rel="noopener">Open Disclosures (PDF)</a>',
                    unsafe_allow_html=True,
                )
            except Exception:
                st.info("Could not create a static link. Use the inline viewer or the download button.")

            # Inline viewer preview (no pop-ups)
            with st.expander("Preview Disclosures (inline)"):
                st.components.v1.html(
                    f'<iframe src="data:application/pdf;base64,{b64}#toolbar=1" width="100%" height="700" frameborder="0"></iframe>',
                    height=720,
                )

            # Optional: allow users to download a copy
            st.download_button(
                "Download PDF",
                data=pdf_bytes,
            ...
            st.info("Add `DataSource LBM Portfolios.pdf` to this app folder (or parent) to enable the link.")
