...
if st.button("Compute required lump sum"):
    if df is None or not allocation_cols:
        ...
        pdf_path = "disclosures.pdf"  # place this file in the same folder as this app
        if os.path.exists(pdf_path):
            with open(pdf_path, "rb") as f:
                pdf_bytes = f.read()
            b64 = base64.b64encode(pdf_bytes).decode()

            # Link that opens the PDF in a new browser tab
            open_link = f'<a href="data:application/pdf;base64,{b64}" target="_blank">Open Disclosures (PDF)</a>'
            ...
            st.download_button(
                ...
                data=pdf_bytes,
                file_name="disclosures.pdf",
                mime="application/pdf",
            )
        else:
            st.info("To enable a disclosures link, add a file named `disclosures.pdf` to this app's folder.")
