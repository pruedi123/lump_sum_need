...
if have_any:
    ...
    # Confidence-based ending value (per source) using lower-tail quantile of ending values
    ...
    q = max(0.0, min(1.0, 1.0 - float(confidence_level)))  # lower-tail percentile
    conf_rows = []
    if src_kind in ("LBM", "BOTH") and df_lbm is not None and allocation_cols_lbm:
        evs_g = _collect_evs(df_lbm, allocation_cols_lbm)
        if evs_g:
            arr = np.sort(np.array(evs_g, dtype=float))
            try:
                ev = np.quantile(arr, q, method="linear")
            except TypeError:
                ev = np.quantile(arr, q, interpolation="linear")
            conf_rows.append({"Source": "Global", "Ending Value": ev * float(goal)})
    if src_kind in ("SPX", "BOTH") and df_spx is not None and allocation_cols_spx:
        evs_s = _collect_evs(df_spx, allocation_cols_spx)
        if evs_s:
            arr = np.sort(np.array(evs_s, dtype=float))
            try:
                ev = np.quantile(arr, q, method="linear")
            except TypeError:
                ev = np.quantile(arr, q, interpolation="linear")
            conf_rows.append({"Source": "SP500", "Ending Value": ev * float(goal)})

    if conf_rows:
        st.markdown("#### Ending Value at Selected Confidence")
        for row in conf_rows:
            st.write(f"{row['Source']}: ${row['Ending Value']:,.0f}")
