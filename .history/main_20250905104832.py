...
with col1:
    ...
    # Currency input with commas + +/- buttons (keeps numeric 'goal' for math)
    GOAL_STEP = 1_000
    GOAL_MIN = 1
    _GOAL_NUM = "goal_num_value"
    _GOAL_STR = "goal_str_value"
    _GOAL_TEXT = "goal_text_input"
    if _GOAL_NUM not in st.session_state:
        st.session_state[_GOAL_NUM] = 1_000_000
    if _GOAL_STR not in st.session_state:
        st.session_state[_GOAL_STR] = f"${st.session_state[_GOAL_NUM]:,.0f}"
    if _GOAL_TEXT not in st.session_state:
        st.session_state[_GOAL_TEXT] = st.session_state[_GOAL_STR]

    left, plus_col, minus_col = st.columns([6, 1, 1])
    with left:
        raw_goal = st.text_input(
            "Goal ($)",
            value=st.session_state[_GOAL_TEXT],
            help=dedent(
    ...
    with plus_col:
        if st.button("+1k", key="goal_plus"):
            st.session_state[_GOAL_NUM] = max(GOAL_MIN, st.session_state[_GOAL_NUM] + GOAL_STEP)
            st.session_state[_GOAL_STR] = f"${st.session_state[_GOAL_NUM]:,.0f}"
            st.session_state[_GOAL_TEXT] = st.session_state[_GOAL_STR]
    with minus_col:
        if st.button("−1k", key="goal_minus"):
            st.session_state[_GOAL_NUM] = max(GOAL_MIN, st.session_state[_GOAL_NUM] - GOAL_STEP)
            st.session_state[_GOAL_STR] = f"${st.session_state[_GOAL_NUM]:,.0f}"
            st.session_state[_GOAL_TEXT] = st.session_state[_GOAL_STR]

    # Parse manual edits (allow $ and commas)
    import re as _re_goal
    digits = _re_goal.sub(r"[^0-9]", "", raw_goal or "")
    if digits:
        st.session_state[_GOAL_NUM] = max(GOAL_MIN, int(digits))
        st.session_state[_GOAL_STR] = f"${st.session_state[_GOAL_NUM]:,.0f}"
    # Normalize display text
    if st.session_state[_GOAL_TEXT] != st.session_state[_GOAL_STR]:
        st.session_state[_GOAL_TEXT] = st.session_state[_GOAL_STR]

    goal = st.session_state[_GOAL_NUM]
with col2:
