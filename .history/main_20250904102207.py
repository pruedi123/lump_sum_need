...
if have_any:
    ...
    if not chart_df.empty:
        fig = go.Figure()
        n = len(chart_df)
        neutral = ["#D3D3D3"] * n  # light gray for non-min bars
        # Highlight colors (complementary-ish pair): Global = teal, SP500 = orange
        highlight_global = "#1B9E77"  # teal/green
        highlight_spx    = "#D95F02"  # orange
        
        if "Global" in chart_df.columns:
            g_vals = chart_df["Global"]
            g_colors = neutral.copy()
            if g_vals.notna().any():
