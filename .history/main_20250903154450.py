...
if st.button("Compute required lump sum"):
    if df is None or not allocation_cols:
        ...
        if pdf_path:
            ...
            # Reliable new-tab open using a Blob URL (avoids about:blank issues with data URLs)
            html_btn = f"""
            <button onclick="(function(){{
              ...
              try {{
                ...
                if (!w) {{ alert('Please allow pop-ups to open the PDF.'); }}
              }} catch (e) {{
                alert('Unable to open PDF in a new tab. You can still use the Download button below.');
              }}
            }})()">Open PDF: {file_label}</button>
            """
            st.components.v1.html(html_btn, height=40)
