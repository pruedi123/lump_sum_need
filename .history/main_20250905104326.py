...
with col1:
    ...
    # Currency-friendly goal input (accepts $ and commas; displays with formatting)
    default_goal = 1_000_000
    goal_key = "goal_input_currency"
    if goal_key not in st.session_state:
        st.session_state[goal_key] = f"${default_goal:,.0f}"
    ...
    # Parse: strip everything except digits
    digits = re.sub(r"[^0-9]", "", raw_goal or "")
    goal = int(digits) if digits else default_goal
    if goal < 1:
        goal = 1
    # Normalize the display to $ with commas
    normalized = f"${goal:,.0f}"
    if raw_goal != normalized:
        st.session_state[goal_key] = normalized
with col2:
