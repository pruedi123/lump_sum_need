...
with col1:
    ...
    essential_goal = st.number_input(
        ...
        help=dedent(
            ...
            "Example: \\$1,000,000 in 25 years means the amount that buys what \\$1,000,000 buys now."
...
with col2:
    ...
    # Acceptable confidence fixed at 100%
    conf_pct_floor = 100
    floor_confidence_level = 1.0
with col3:
    fee_pct = st.slider(
        "Annual fee (%)",
        min_value=0.0,
...
if have_any:
    ...
    if src_kind in ("LBM", "BOTH") and df_lbm is not None:
        for col in allocation_cols_lbm:
            ...
            if not evs:
                ...
                req_essential = required_lumpsum_for_goal(evs, float(essential_goal), float(essential_confidence_level))
                req_floor = required_lumpsum_for_goal(evs, float(floor_goal), float(floor_confidence_level))
                req = max(req_essential, req_floor)
                note = ""
            rows.append({
    ...
    # CAGR = (essential_goal / required_lump_sum) ** (1/years) - 1
    ...
    if "Global" in wide.columns:
        with np.errstate(invalid="ignore", divide="ignore"):
            wide["Global CAGR"] = np.where(
                ...
                (essential_goal / wide["Global"]) ** (1.0 / years_float) - 1.0,
                np.nan,
            )
    if "SP500" in wide.columns:
        with np.errstate(invalid="ignore", divide="ignore"):
            wide["SP500 CAGR"] = np.where(
                ...
                (essential_goal / wide["SP500"]) ** (1.0 / years_float) - 1.0,
                np.nan,
            )
    ...
    st.caption("Required Lump Sum satisfies BOTH: Essential Goal at Essential Confidence AND Floor Goal at Floor Confidence. CAGRs are implied from the Essential Goal.")
    ...
    def _failure_stats(df_src, raw_col, required_amt, label_source):
        ...
        total = int(arr.size)
        fails = arr < float(essential_goal)
    ...
    def _success_stats(df_src, raw_col, required_amt, label_source):
        ...
        total = int(arr.size)
        succ_mask = arr >= float(essential_goal)
